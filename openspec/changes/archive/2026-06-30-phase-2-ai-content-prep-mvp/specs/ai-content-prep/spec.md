...
定义 AI 辅助内容加工管线：把**已确认的**真实导入数据，经离线 `services/ai-worker` 生成为 `validating` 草稿，由 **BFF** 从持久化行重建后经三道确定性硬门分流 `draft`/`quarantine`，人工**逐包**确认后才进入正式学习流。范围限定为单一生成类型（单知识点 → 一个引用真实已确认导入客观题的 draft 课包）。
